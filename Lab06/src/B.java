...
public class B {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            graph.a[i] = scanner.nextInt();
            graph.b[i] = scanner.nextInt();
        }
    ...
    private static class Dijkstra {
        ...
        private void relax(EdgeWeightGraph graph, int node) {
            for (DirectedEdge e : graph.adj[node]) {
                int newTime = distTo[e.from] + e.weight;
                int remain = newTime % (graph.a[e.to] + graph.b[e.to]);
                newTime += Math.max(0, graph.a[e.to] - remain);

                if (distTo[e.to] == INFINITY || distTo[e.to] > newTime) {
